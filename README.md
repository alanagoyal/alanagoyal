...
- natural-language photo search ranked by Jev
...
- one AI-generated search description per photo in the database
...
- Jev ranks the search descriptions; Jev receives only text metadata, never image URLs
...
new uploads also get one short description for natural-language search. after deploying the search migration, describe existing photos in small batches until `remaining` is `0`:
...
# rank photo-search candidates with jev
...
- `OPENAI_API_KEY` is used once per photo to create its search description and collection
- `TYPESAFE_API_KEY` enables Jev ranking; search falls back to local text matching if Jev is temporarily unavailable
