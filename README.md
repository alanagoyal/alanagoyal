...
- natural-language photo search with Jev relevance reranking
- upload via ios shortcut with searchable ai metadata and auto-categorization
...
- searchable metadata (caption, tags, visible text, filename, timestamp, collections) and embeddings in database
...
- upload via api with searchable metadata generated by OpenAI vision
- semantic retrieval with pgvector and optional Jev reranking; Jev receives only the query and candidate metadata, never image URLs
...
new uploads are also captioned and embedded for natural-language search. after deploying the semantic-search migration, index existing photos in small batches until `remaining` is `0`:

```bash
curl -X POST https://yourdomain.com/api/photos/reindex \
  -H "x-api-key: $PHOTOS_UPLOAD_API_KEY" \
  -H "content-type: application/json" \
  -d '{"limit":5}'
```
...
# optional: rerank semantic photo-search candidates with jev
...
- `OPENAI_API_KEY` is used for photo captions, tags, visible-text extraction, and search embeddings
- `TYPESAFE_API_KEY` enables Jev reranking; photo search falls back to vector similarity when it is absent or Jev is unavailable
