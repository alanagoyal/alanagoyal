...
- `components/desktop/desktop.tsx` treats `TextEdit` and `Preview` as Finder-routed apps when no document is available.
...
3. Open a text file from Finder and confirm it opens in a `TextEdit` window.
4. Edit that text file, minimize and restore the window, and confirm the content stays in sync for that file path.
5. Open an image or PDF from Finder and confirm it opens in a `Preview` window with the existing viewer behavior.
6. Navigate directly to `/textedit?file=<valid text file>` and `/preview?file=<valid image-or-pdf>` and confirm deep links still work.
